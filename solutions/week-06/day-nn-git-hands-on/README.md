...
> Well, written in md, coz, its better read for the people who are verifying this, module completion
> `¯\_(ツ)_/¯`
...
```bash
git add . # Here dor (.) indicates the currently working directory
# few other basic commands
git push
git reflog
git status
git log --oneline # just in online

git log --oneline | wc -l # this gives you total no.of commits in linux (wc cli tool - word count) 
